...
const getAllServiceFromDB = async (payload: any, query: any) => {
  ...
  if (payload?.coordinates && payload?.distance) {
    ...
    matchStage.coordinates = {
      $geoWithin: { $centerSphere: [[lng, lat], payload.distance / 6378137] },
    };
  }
  ...
  // Fetch bookmarks for only the returned services
  const serviceIds = results.map((s) => s._id);
  ...
  // Add isFavorite to paginated results
  const servicesWithFavorite = results.map((s) => ({
    ...s,
