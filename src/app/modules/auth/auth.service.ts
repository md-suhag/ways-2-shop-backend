...
const loginUserFromDB = async (payload: ILoginData) => {
  ...
  if (!isExistUser.password && isExistUser.appId) {
    throw new ApiError(
      StatusCodes.BAD_REQUEST,
      "please  use continue with google, because you logged in with google previously"
    );
  }
...
const changePasswordToDB = async (
  ...
  if (
    ...
    !(await User.isMatchPassword(
      currentPassword,
      isExistUser.password ? isExistUser.password : ""
    ))
  ) {
    throw new ApiError(
      StatusCodes.BAD_REQUEST,
...
const resendVerificationEmailToDB = async (email: string) => {
  ...
  const resendOtpEmailTemplate = emailTemplate.resendOtpEmail(emailValues);
